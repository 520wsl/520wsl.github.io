<template>
    <div>
        <Title :titleObj="titleObj"></Title>
        <downloadAddress :addressObj="addressObj"></downloadAddress>
        <stepList :installationList="installationList"></stepList>
    </div>
</template>
<script>
    import Title from "@/components/workOrder/download/title";
    import downloadAddress from "@/components/workOrder/download/download-address";
    import stepList from "@/components/workOrder/download/step-list";
    import {getDownloadInfo} from "@/workOrder/api/workOrder/download";

    export default {
        data() {
            return {
                titleObj: {
                    logo: this.$CDN('/download/download-title-logo.png'),
                    title: '喜小帮桌面版客户端下载'
                },
                addressObj: {
                    backgroundUrl: this.$CDN('/download/download-background.png'),
                    title: '“喜小帮”是杭州四喜的客户服务平台',
                    advantage: '减少沟通成本，提升服务质量，全流程跟踪，数据化管理',
                    downloadList: [
                        // {
                        //     type: 'Win64',
                        //     logoUrl: this.$CDN('/download/microsoft-icon.png'),
                        //     downloadUrl: 'http://custom-center.oss-cn-hangzhou.aliyuncs.com/customerCenter/electron/win32_x64/xxb_win_1.0.4.exe',
                        //     title: 'Windows客户端',
                        //     isRecommend: true,
                        //     latestVersion: '最新版本：V1.0.4',
                        //     applicablePlatform: '适用平台：Win7/8/10    64位'
                        // },
                        // {
                        //     type: 'Win32',
                        //     logoUrl: this.$CDN('/download/microsoft-icon.png'),
                        //     downloadUrl: 'http://custom-center.oss-cn-hangzhou.aliyuncs.com/customerCenter/electron/win32_ia32/xxb_win_1.0.4.exe',
                        //     title: 'Windows客户端',
                        //     isRecommend: false,
                        //     latestVersion: '最新版本：V1.0.4',
                        //     applicablePlatform: '适用平台：Win7/8/10    32位'
                        // },
                        // {
                        //     type: 'Linux',
                        //     logoUrl: this.$CDN('/download/linux-icon.png'),
                        //     downloadUrl: 'http://custom-center.oss-cn-hangzhou.aliyuncs.com/customerCenter/electron/linux_x64/xxb_linux_1.0.4.AppImage',
                        //     title: 'Linux客户端',
                        //     isRecommend: false,
                        //     latestVersion: '最新版本：V1.0.4',
                        //     applicablePlatform: '',
                        //     description: ['支持Ubuntu、Arch Linux等']
                        // },
                        // 目前暂无ISO系统端 先隐藏
                        // {
                        //     logoUrl: this.$CDN('/download/iso-icon.png'),
                        //     downloadUrl: '',
                        //     title: '苹果系统客户端',
                        //     isRecommend: false,
                        //     latestVersion: '最新版本：V1.0.0',
                        //     applicablePlatform: '适用平台：Mac osx 10.12+',
                        //     description: []
                        // }
                    ]
                },
                installationList: [
                    {
                        imgUrl: this.$CDN('/download/step-1.png'),
                        stepInfo: '第一步：双击exe文件'
                    },
                    {
                        imgUrl: this.$CDN('/download/step-2.png'),
                        stepInfo: '第二步：选择安装路径（也可以使用默认安装路径）'
                    },
                    {
                        imgUrl: this.$CDN('/download/step-3.png'),
                        stepInfo: '第三步：完成安装'
                    }
                ]
            }
        },
        components: {
            Title,
            downloadAddress,
            stepList
        },
        methods: {
            async getDownloadInfoAction() {
                let res = await getDownloadInfo()
                if (!res || res.status !== 200) {
                    console.log('系统信息获取失败')
                    return;
                }
                let arr = []
                let data = res.data
                for (let item in data) {
                    let obj = {}
                    obj['type'] = item || '';
                    obj['logoUrl'] = data[item]['logoUrl'] || "";
                    obj['downloadUrl'] = data[item]['file'] && data[item]['file']['path'] || ""
                    obj['title'] = data[item]['title'] || ""
                    obj['isRecommend'] = (new RegExp(data[item]['navigator'], 'i')).test(navigator.userAgent)
                    obj['latestVersion'] = data[item]['file'] && data[item]['file']['version'] || ""
                    obj['applicablePlatform'] = data[item]['applicablePlatform'] || ""
                    obj['description'] = data[item]['description'] || []
                    if (obj['isRecommend']) {
                        arr.unshift(obj)
                    } else {
                        arr.push(obj)
                    }
                }
                console.log('arr', arr)
                this.addressObj.downloadList = arr
            }
        },
        mounted() {
            this.getDownloadInfoAction()
        }
    }
</script>
<style scoped>
</style>
